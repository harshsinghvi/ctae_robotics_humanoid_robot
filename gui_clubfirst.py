...
def recognize_speech_from_mic(recognizer, microphone):
    ...
    with microphone as source:
        ...
        speech_text = 'Please Speak Close to my camera'
        engine.say(speech_text)
        # window.write_event_value('-THREAD-', speech_text)
        print(speech_text)
        engine.runAndWait()
...
def mic_listen():
    ...
    if guess["error"]:
        ...
        return guess
    speech_text = "You said: {}".format(guess["transcription"]) 
    # engine.say(speech_text)# repeating question
    # window.write_event_value('-THREAD-', speech_text)
    #print(speech_text)
    print(speech_text)
    # engine.runAndWait()
...
def assistant(name, face_encoding):
    ...
    if name == "Guest":
        ...
        speech_text = "Welcome Guest. May I register your details? or continue"
        engine.say(speech_text)
        print(speech_text)
        engine.runAndWait()
        # red.off()
        ...
        if event == sg.WIN_CLOSED:
            return True
        if "register" in guess["transcription"].lower():
            ...
            if ("confirm" in confirm["transcription"].lower()) or ("yes" in confirm["transcription"].lower()):
                ...
                print(speech_text)
                engine.runAndWait()
                name = sg.popup_get_text(message='Please enter your name', title='Name correction', keep_on_top=True, location=(0, 200))
            ...
            if ("confirm" in confirm["transcription"].lower()) or ("yes" in confirm["transcription"].lower()):
                ...
                print(speech_text)
                engine.runAndWait()
                mobile = sg.popup_get_text(message='Please enter your Mobile Number', title='Number correction', keep_on_top=True, location=(0, 200))
            ...
            url = 'https://clubfirstrobotics.com/utility/API/user.php'
    ...
    speech_text = "Hello " + name + "How can I help you? "
    engine.say(speech_text)
    # window.write_event_value('-THREAD-', speech_text)
    print(speech_text)
    engine.runAndWait()
    ...
    if event == sg.WIN_CLOSED:
        return True
    if "complain" in guess["transcription"].lower():
        # print("complaint")
        ...
        if comp["error"]:
            return False
        url = 'https://clubfirstrobotics.com/utility/API/complaint.php'
        myobj = {'CUSID': 1, 'DEVID': 1, 'C1': comp["transcription"]}
        x = requests.post(url, data=myobj, timeout=5)
        ...
        print(speech_text)
        engine.runAndWait()
    elif "game" in guess["transcription"].lower():
        # print("game")
        game()
    elif "feedback" in guess["transcription"].lower():
        # print("feedback")
        ...
        if rating["error"]:
            return False
        url = 'https://clubfirstrobotics.com/utility/API/feedback.php'
        myobj = {'CUSID': 1, 'DEVID': 1, 'M1': feedback["transcription"], 'M2': rating["transcription"]}
        x = requests.post(url, data=myobj, timeout=5)
        ...
        print(speech_text)
        engine.runAndWait()
    elif "tour" in guess["transcription"].lower():
        # print("feedback")
        ...
        if "home".lower() in goal_pos["transcription"].lower():
        ...
        elif "student".lower() in goal_pos["transcription"].lower():
            '''speech_text = "Are you sure you want me to take you to there?"
            engine.say(speech_text)
            # window.write_event_value('-THREAD-', speech_text)
            print(speech_text)
            engine.runAndWait()
            ...
            '''speech_text = "Are you sure you want me to take you to three?"
            engine.say(speech_text)
            # window.write_event_value('-THREAD-', speech_text)
            print(speech_text)
            engine.runAndWait()
